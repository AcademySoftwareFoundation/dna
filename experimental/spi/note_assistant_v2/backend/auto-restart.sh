...
# Auto-restart script for Note Assistant v2 Backend
# Automatically restarts "npm run dev" when it crashes
# Press Ctrl-C to stop and exit

echo "Note Assistant v2 Backend Auto-Restart"
echo "======================================"
...
while true; do
    if [ $restart_count -eq 0 ]; then
        ...
        echo "Restart #$restart_count - Starting backend server..."
    fi
    ...
    if [ $exit_code -eq 130 ]; then
        echo "Backend server interrupted."
        break
    fi
    ...
    echo ""
    echo "⚠️  Backend server crashed with exit code: $exit_code"
